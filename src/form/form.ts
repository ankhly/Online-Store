const formHtml = `
  <div class="main-page__form form-page">
    <form class="form-page__body">
      <div class="form-page__details input">
      ...
      <div class="form-page__credit card">
      ...
      <button type="submit" class="form-page__btn">Confirm</button>
    </form>
  </div>
`;
...
const validateValid = (array:Array<string>) => {
  ...
  return arr[0].length === 2 && Number(arr[0]) <= 12 && array.join('').length === 5 && arr[1] >= year && Number(arr[0]) >= mon;
};
...
export function btnClick():void {
  ...
  document.addEventListener('click', (e:Event):void => {
    if (e.target === buyBtn) {
      mainPage.insertAdjacentHTML('beforeend', formHtml);
      wrapper.classList.add('bg');
      const form = document.querySelector('.form-page__body') as HTMLFormElement;
      if (form) {
